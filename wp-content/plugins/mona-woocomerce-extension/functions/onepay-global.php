...
function init_Mona_OnePay_global_Domestic_class() {

  
    class Mona_OnePay_global_Domestic extends WC_Payment_Gateway {
    ...
    public function __construct() {
        $this->id = 'mona_onepay_global';
        $this->has_fields = false;
        $this->order_button_text = __('Proceed to OnePay', 'monamedia');
        $this->method_title = __('OnePay Domestic Gateway (by Monamedia)', 'monamedia');
        $this->method_description = __('OnePay supports Gobal payment', 'monamedia');
        $this->supports = array(
            'products',
        );
    ...
    public function init_form_fields() {
        $this->form_fields = array(
                'enabled' => array(
                    ...
                    'type' => 'checkbox',
                    'label' => __('OnePay Domestic Gateway (by Mona Media)', 'monamedia'),
                    'default' => 'no'
                ),
                'testmode' => array(
                    'title' => __('OnePay Sandbox', 'monamedia'),
                    'type' => 'checkbox',
                ...
                'title' => array(
                    ...
                    'default' => __('OnePay Domestic Gateway', 'monamedia'),
                    'desc_tip' => true,
                ),
                'description' => array(
                    ...
                    'default' => __('With OnePay, you can make payment by using any local Vietnam ATM card.', 'monamedia')
                ),
                'api_details' => array(
                    'title' => __('API Credentials', 'monamedia'),
                    'type' => 'title',
                    'description' => sprintf(__('Enter your OnePay API credentials. Contact OnePay to have your credentials %shere%s.', 'monamedia'), '<a href="http://onepay.com.vn/home/en/contact-us.html">', '</a>'),
                ),
                'merchant_id' => array(
                    'title' => __('Merchant ID', 'monamedia'),
                    'type' => 'text',
                    'description' => __('Get your Merchant ID from OnePay.', 'monamedia'),
                    'default' => '',
                    'desc_tip' => true,
                    'placeholder' => __('Required. Provided by OnePay.', 'monamedia')
                ),
                'access_code' => array(
                    'title' => __('Access Code', 'woocommerce'),
                    'type' => 'text',
                    'description' => __('Get your Access Code from OnePay.', 'monamedia'),
                    'default' => '',
                    'desc_tip' => true,
                    'placeholder' => __('Required. Provided by OnePay.', 'monamedia')
                ),
                'secure_secret' => array(
                    ...
                    'type' => 'text',
                    'description' => __('Get your Secure Secret from OnePay.', 'monamedia'),
                    'default' => '',
                    'desc_tip' => true,
                    'placeholder' => __('Required. Provided by OnePay.', 'monamedia')
                ),
                'user' => array(
                    ...
                    'type' => 'text',
                    'description' => __('Get your user info from OnePay.', 'monamedia'),
                    'default' => '',
                    'desc_tip' => true,
                    'placeholder' => __('Required. Provided by OnePay', 'monamedia')
                ),
                'password' => array(
                    ...
                    'type' => 'text',
                    'description' => __('Get your password info from OnePay.', 'monamedia'),
                    'default' => '',
                    'desc_tip' => true,
                    'placeholder' => __('Required. Provided by OnePay.', 'monamedia')
                ),
                'more_info' => array(
                    'title' => __('Instant Payment Notification (IPN)', 'monamedia'),
                    'type' => 'title',
                    'description' =>
                    sprintf('URL: <code>%s</code>', Mona_OnePay_global_Domestic::get_onepay_ipn_url()) . '<p/>' .
                    sprintf(__('%sContact OnePay%s to configure this URL on its site. <strong>This is required  based on its guidelines.</strong>', 'monamedia'), '<a href="http://onepay.com.vn/home/en/contact-us.html">', '</a>'),
                ),
                /**
                 * @since 1.3.1
                 */
                'debug' => array(
                    'title' => __('Debug log', 'monamedia'),
                    'type' => 'checkbox',
                    'label' => __('Enable logging', 'monamedia'),
                    'default' => 'no',
                    'description' => sprintf(__('Log events, such as IPN requests, inside %s', 'monamedia'), '<code>' . WC_Log_Handler_File::get_log_file_path('Mona_OnePay_global_Domestic') . '</code>'),
                ),
            );
    }
    ...
    public function get_pay_url($order) {
         $settings = Mona_extension::get_settings();
          ...
          if($symbol!='VND'){
              $total = $total*$settings['vnd_paypal_standard']['rate'];
          }
         
        $args = array(
            'Title' => __('OnePay Payment Title', 'monamedia'),
            ...
            'vpc_OrderInfo' => substr(
                    sprintf('Order #%1$s - %2$s', $order->get_id(), get_home_url()), 0, 32), // Limit 32 characters
            'vpc_Amount' => $total, // Multiplying 100 is a requirement from OnePay
            'vpc_ReturnURL' => $this->get_return_url($order),
          //  'vpc_ReturnURL' => wc_get_checkout_url(),
            'vpc_Version' => '2',
            'vpc_Command' => 'pay',
            'vpc_Locale' => ( 'vi' == get_locale() ) ? 'vn' : 'en',
            'vpc_Currency' => 'VND',
            'AgainLink' =>esc_url(wc_get_checkout_url() ),
            'vpc_TicketNo' => @$_SERVER['REMOTE_ADDR'],
            'AVS_City' => $order->get_billing_city(),
            'AVS_Country' => $order->get_billing_country(),
            'AVS_PostCode' => $order->get_billing_postcode(),
            'AVS_StateProv' => $order->get_billing_state(),
            'AVS_Street01' => $order->get_billing_address_1(),
            'vpc_Customer_Email' => $order->get_billing_email(),
            'vpc_Customer_Phone' => $order->get_billing_phone(),
            'vpc_SHIP_City' => $order->get_shipping_city(),
            'vpc_SHIP_Country' => $order->get_shipping_country(),
            'vpc_SHIP_Provice' => $order->get_shipping_state(),
            'vpc_SHIP_Street01' => $order->get_shipping_address_1(),
        );
        ...
        if ($this->testmode) {
            ...
            return 'https://onepay.vn/vpcpay/vpcpay.op?' . $http_args;
        }
    }
    ...
    public function set_onepay_querydr_cron($vpc_MerchTxnRef) {
        ...
        wp_schedule_single_event(
                time() + 20 * 60, 'mona_handle_onepay_querydr', array($vpc_MerchTxnRef)
        );
    }
    ...
    public function process_onepay_response_data($args, $type) {
        ...
        if ($is_secure OR $is_querydr_exists) {
            ...
            $order_note = sprintf(
                    __('OnePay Domestic Gateway Info | Code: %1$s | Message: %2$s | MerchantTxnRef: %3$s | Type: %4$s', 'monamedia'), $vpc_TxnResponseCode, $this->OnePay_getResponseDescription($vpc_TxnResponseCode), $vpc_MerchTxnRef, $type
            );
    ...
    function add_one_pay_global_gateway_class($methods) {
        $methods[] = 'Mona_OnePay_global_Domestic';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'add_one_pay_global_gateway_class');
}
