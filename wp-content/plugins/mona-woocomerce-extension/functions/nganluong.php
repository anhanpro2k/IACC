...
function mona_gateway_payment_nganluong_init() {
    if (!class_exists('WC_Payment_Gateway'))
        return;

    class mona_Gateway_NganLuong extends WC_Payment_Gateway {
        ...
        function __construct() {
          ...
          //  $this->debug = $this->settings['debug'];
            $this->order_button_text = __('Thanh Toán Qua Ngân Lượng', 'woocommerce');
            ...
            add_action('parse_request', array($this,'nganluong_return_handler'));
           // add_action('woocommerce_api_mona_gateway_nganluong', array($this, 'callback'));
        ...
        public function init_form_fields() {
			  $str = get_site_url() . '/nganluong_return';
            // Admin fields
            $this->form_fields = array(
                'enabled' => array(
                    'title' => __('Activate', 'woocommerce'),
                    'type' => 'checkbox',
                     'label' => __('Activate the payment gateway for Ngan Luong</br> Ngân Lượng Url Config: '.$str, 'woocommerce'),
                    'default' => 'yes'),
                'title' => array(
    ...
    function mona_paymen_add_NganLuong_gateway($methods) {
        $methods[] = 'mona_Gateway_NganLuong';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'mona_paymen_add_NganLuong_gateway');
}
