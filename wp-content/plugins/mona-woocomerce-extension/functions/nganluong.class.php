<?php
class mona_nganluong_listener extends mona_Gateway_nganluong
{
	private $file_log_name = FILE_NAME;
	private $test_bpn = 'http://sandbox.nganluong.vn:8088/nl30/checkout.api.nganluong.post.php';
	private $live_bpn = 'https://www.nganluong.vn/checkout.api.nganluong.post.php';
	private $myFile;
        private $testmode;
	...
	/**
	 * Hàm thực hiện nhận và kiểm tra dữ liệu từ Ngân Lượng
	 * @return bool
	 */
	private function check_bpn_request_is_valid()
	{
		...
		//Kiểm tra sự tồn tại dữ liệu nhận từ nganluong
		if (empty($_POST)) {
			$this->writeLog("Khong nhan duoc du lieu tu nganluong");
			return false;
		}

		//Lấy url verify BPN
		if ($this->testmode == 'yes'):
			$nganluong_url = $this->test_bpn;
		else :
			$nganluong_url = $this->live_bpn;
		endif;
		...
		if ($this->_isCurl()) {
			...
			$this->writeLog('Gui du lieu den '.$nganluong_url.'. Thuc hien kiem tra tinh hop le cua BPN...', true);
		} else {
	 ...
	 *          - Số tiền giao dịch.
	 *
	 * @param $transaction_status Trạng thái giao dịch từ nganluong
	 *                           4 : giao dịch hoàn thành
	 *                          13 : Giao dịch tạm giữ
	 *
	 * @param $total_amount     Số tiền thanh toán ở nganluong
	 * @param $order_id         Mã đơn hàng thanh toán từ nganluong
	...
	private function isValidOrderInfo($transaction_status, $total_amount, $order_id)
	{
		...
		$success_transaction_status = array(nganluong_TRANSACTION_STATUS_COMPLETED, nganluong_TRANSACTION_STATUS_TEMP_HOLDING);
