<?php
class Mona_Shipping_VN_POST extends WC_Shipping_Method
{
    ...
    public function __construct($instance_id = 0)
    {
        ...
        $this->_user = get_option('m_vn_post_user_name');
        $this->_password = get_option('m_vn_post_user_password');
        ...
        $live = get_option('m_vn_post_products');
        $this->_live = false;
        if ($live == 'yes') {
            $this->_live = true;
        }
        $SenderProvinceId = get_option('m_vn_post_provinces_user');
        $SenderDistrictId = get_option('m_vn_post_district_user');
        ...
        $this->_url_api = 'https://donhang-uat.vnpost.vn/api/api/';
        if ($this->_live) {
            $this->_url_api = 'https://donhang.vnpost.vn/api/api';
        }
        ...
        $this->method_title = __('VNPost: Bưu điện Việt Nam', 'monamedia');
        $this->method_description = __('Giao Hàng Qua <a target="_blank" href="http://www.vnpost.vn/">VNPost: Bưu điện Việt Nam</a>', 'monamedia');
        $this->supports = array(
            'shipping-zones',
            'instance-settings',
    ...
    public function calculate_shipping($package = array())
    {
        ...
        $rate = array(
            'id' => $this->get_rate_id() . '-2',
            'label' => 'VNPOST: Giao thường',
            'cost' => $cost['BK'],
            'package' => $package,
        );
        $this->add_rate($rate);
    ...
    public function render_shipping_price($package)
    {
        ...
        $api = 'CustomerConnect/TinhCuocTatCaDichVu';
        global $woocommerce;
        ...
        if (is_array($items) && count($items) > 0) {
            ...
            $stringCity = $this->slugify($city . ' ' . $stated);
            $stringAddress = $this->slugify($address);
            $Receiver = $this->get_province_id($stringCity, $stringAddress);
            // var_dump($Receiver);
            // exit;
            if (!$Receiver) {
                return;
            ...
            $length = 1;
            $height =  1;
            $codAmount = 0; # tiền thu hộ COD; > 0 coi như là sử dụng
            $IsReceiverPayFreight = true; # Cộng thêm cước vào tiền thu hộ - nếu giá trị là true thì mặc định khách sử dụng dịch vụ giao hàng thu tiền (COD)
            $OrderAmount = $package['cart_subtotal'];
            $UseBaoPhat = false; # dùng dịch vụ báo phát hay không
            $UseHoaDon = false; # Có dùng dịch vụ hóa đơn hay không?
            ...
            $data = array(
                ...
                "UseBaoPhat" => $UseBaoPhat,
                "UseHoaDon" => $UseHoaDon,
                "UseNhanTinSmsNguoiNhanTruocPhat" => false,
                "UseNhanTinSmsNguoiNhanSauPhat" => false,
            );

            $url = $this->_url_api . $api;
            $method = "POST";
            $postField = $data;

            // $response = $this->call_cURL($url, $method, $headers, $postField); 
            $curl = curl_init();
            ...
            if ($err) {
                ...
                $res = [];
                // return (float) $result[0]->TongCuocBaoGomDVCT;
                if (is_array($result)) {
                    foreach ($result as $k => $value) {
                        $res[$value->MaDichVu] = $value->TongCuocBaoGomDVCT;
                    }
                }
                return $res;
            }
    ...
    {
        $url = "/QuanHuyen/Getall";
        $response = $this->call_cURL($url, "GET", [], false);
        $result = false;
        if ($response['status'] == 200) {
            $data = json_decode($response['data']);
            if (is_array($data)) {
                foreach ($data as $key => $value) {
                    $name = $value->TenDayDuKhongDau;
                    if (strpos($string, $name) or $string == $name) {
                        $result = ['MaQuanHuyen' => $value->MaQuanHuyen, 'MaTinhThanh' => $value->MaTinhThanh];
                        return $result;
                        break;
                    }
                }
    ...
    {
        $url = 'MobileAuthentication/GetAccessToken';
        $method = "POST";
        $postField = [
            'TenDangNhap' => $this->_user,
            'MatKhau' => $this->_user,
        ];
        $headers =  [
        ...
        if ($result['status'] == 200) {
            $arrData = json_decode($result['data']);
